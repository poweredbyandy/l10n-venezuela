...
class PosOrder(models.Model):
    ...
    def l10n_ve_fiscal_serial_check_print_move(self):
        ...
        raise UserError(
            _(
                "Tipo de documento %(move_type)s no soportado para impresión fiscal desde POS."
            )
    ...
    def l10n_ve_fiscal_serial_register_print_result(self, values):
        ...
        raise UserError(
            _(
                "Tipo de documento %(move_type)s no soportado para registrar impresión fiscal desde POS."
            )
            % {"move_type": move.move_type or ""}
        )
