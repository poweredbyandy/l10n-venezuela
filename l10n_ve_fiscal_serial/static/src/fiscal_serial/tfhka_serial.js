...
export class TfhkaFiscal {
    ...
    _darStatusError(st, er) {
        ...
        this.descripStatus =
            STATUS_MESSAGES[st] || `Estado código ${st}`;
    ...
    async _serialPortWriteAndRead(payload, readMode, ioOptions = {}) {
        ...
        const bt =
            bulkReadOptions.byteTimeout ??
            Math.max(10, this._serialPortReceiveTimeout);
        const totalT =
    ...
    async _serialPortEnqQuery(enqRead) {
        ...
        let { n, bytes } = await this._serialPortWriteAndRead(
            payload,
            "bulk",
            enqRead
        );
        if (n < 0) {
            ({ n, bytes } = await this._serialPortWriteAndRead(
                payload,
                "bulk",
                enqRead
            ));
    ...
    async openFpCtrl(first, second) {
        ...
        let baudRate = 9600;
        let parity = "even";
        if (first != null && typeof first === "object" && first.readable !== undefined) {
            port = first;
    ...
    async readFpStatus() {
        ...
        try {
            ...
            let bResp;
            try {
                ({ n: num1, bytes: bResp } = await this._serialPortEnqQuery(
                    ENQ_READ_OPTS
                ));
            } finally {
                this._serialPortReceiveTimeout = prev;
