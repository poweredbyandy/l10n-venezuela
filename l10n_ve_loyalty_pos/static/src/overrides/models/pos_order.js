...
patch(PosOrder.prototype, {
    ...
    _l10nVeGetEwalletSpendLines() {
        return (this.lines || []).filter((line) => this._l10nVeIsEwalletRewardLine(line));
    },
    ...
    _l10nVeRoundInCurrency(amount, currency) {
        const resolved = this._l10nVeResolveCurrency(currency) || this._l10nVeGetOrderCurrency();
    ...
    pointsForPrograms(programs) {
        ...
        for (const program of programs || []) {
            ...
            if (!orderCurrency || !walletCurrency || orderCurrency.id === walletCurrency.id) {
                continue;
            }
            result[program.id] = result[program.id].map((entry) => ({
                ...entry,
                points: this._l10nVeRoundInCurrency(
                    this._l10nVeConvertAmount(entry.points, orderCurrency, walletCurrency),
                    walletCurrency
                ),
            }));
        }
    ...
    _l10nVeBuildWalletGlobalDiscountLines(args) {
        ...
        const result = Object.entries(discountablePerTax).reduce((lines, entry) => {
            ...
            let taxIds =
                entry[0] === "" ? [] : entry[0].split(",").map((str) => parseInt(str, 10));
    ...
    _l10nVeComputeTaxTotalsWithManualDiscounts() {
        ...
        baseLines.push(...this._l10nVeBuildManualDiscountBaseLines(documentSign, manuals));
