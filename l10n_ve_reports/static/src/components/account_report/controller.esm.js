...
export class AccountReportController {
    ...
    async preLoadClosedSections() {
        ...
        const self = this;
        globalThis.setTimeout(() => self.preLoadClosedSections(), nextCallDelay);
    }
    ...
    async loadReportOptions(
        ...
        if (!this.reportOptionsMap[cacheKey]) {
            ...
            else if (this.lastOpenedSectionByReport[reportId]) {
    ...
    areLinesOrdered() {
        return (
            this.linesOrder !== null &&
            this.linesOrder !== undefined &&
            this.options.order_column !== null &&
    ...
    **/
    ...
    hideZeroLines(lines) {
        ...
        reversed_lines.forEach((line) => {
            const isZero = line.columns.every(
                (column) =>
                    !number_figure_types.includes(column.figure_type) || column.is_zero
            );
