...
export class AccountReportFilters extends Component {
    ...
    initDateFilters() {
        ...
        const fallback = specifier === "previous" ? -1 : specifier === "next" ? 1 : 0;
    ...
    _changePeriod(periodType, increment) {
        ...
        this.dateFilter[periodType] += increment;
    ...
    _displayTaxPeriod(dateTo) {
        ...
        const targetDateInPeriod = dateTo.plus({
            months: periodicitySettings.months_per_period * this.dateFilter.tax_period,
        });
    ...
    async applyFilters(optionKey = null, delay = 500) {
        ...
        this.timeout = globalThis.setTimeout(async () => {
    ...
    async onCurrencyRateDateChanged(date) {
        ...
        if (date instanceof DateTime) {
            ...
            try {
                ...
                globalThis.console.error("Error converting date:", e);
                return;
            }
        }
