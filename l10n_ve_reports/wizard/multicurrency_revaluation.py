...
class MulticurrencyRevaluationWizard(models.TransientModel):
    ...
    date = fields.Date(
        default=lambda self: self._context["multicurrency_revaluation_report_options"][
            "date"
        ]["date_to"],
        required=True,
    )
    ...
    def default_get(self, default_fields):
        ...
        if "reversal_date" in default_fields:
            ...
            rec["reversal_date"] = fields.Date.to_date(
                report_options["date"]["date_to"]
            ) + relativedelta(days=1)
        if (
            not self._context.get("revaluation_no_loop")
    ...
    def _get_move_vals(self):
        ...
        options = {
            **self._context["multicurrency_revaluation_report_options"],
            "unfold_all": False,
        }
