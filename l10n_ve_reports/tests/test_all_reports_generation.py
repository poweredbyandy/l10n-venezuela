...
class TestAllReportsGeneration(AccountTestInvoicingCommon):
    ...
    def test_generate_all_export_files(self):
        ...
        l10n_cz_reports_tax_office = self.env.ref(
            "l10n_cz_reports_2025.tax_office_1", raise_if_not_found=False
        )
        company_test_values = {
            "LU": {"vat": "LU12345613"},
            ...
            "AE": {
                ...
                "l10n_ae_tax_report_counterpart_account": l10n_ae_liability_account,
            },
            "CZ": {
        ...
        for report in self.reports:
                ...
                for option_button in options["buttons"]:
                    ...
                    with self.subTest(button=option_button["name"]):
                        with patch.object(
                            ...
                            "_run_wkhtmltopdf",
                            lambda *args, **kwargs: b"This is a pdf",
                        ):
            ...
            self.env.company.write(
                {
                    field_name: None
                    for field_name in company_test_values.get(
                        report.country_id.code, {}
                    ).keys()
                }
            )
            ...
            self.env.company.partner_id.write(
                {
                    field_name: None
                    for field_name in partner_test_values.get(
                        report.country_id.code, {}
                    ).keys()
                }
            )
