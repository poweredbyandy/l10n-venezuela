...
class TestBalanceSheetBalanced(TestAccountReportsCommon):
    ...
    def _set_up_localization(self, coa):
        ...
        credit_move_aml_pairs, credit_accounts_by_aml = (
            self._create_balance_sheet_test_move(coa_setup_data, create_credits=True)
        )
    ...
    def _get_report_totals(self, report_setup_data):
        ...
        def get_line_amount(lines, report_line, balance_column_idx):
            line = next(
                filter(
                    lambda line: self.env["account.report"]._get_res_id_from_line_id(
