...
class TestReconciliationReport(TestAccountReportsCommon):
    ...
    def test_reconciliation_report_single_currency(self):
        ...
        payment_line = payment_1.move_id.line_ids.filtered(
            ...
            == payment_1.payment_method_line_id.payment_account_id
        )
        wizard = (
        ...
        payment_line = payment_2.move_id.line_ids.filtered(
            lambda line: line.account_id
            == payment_2.payment_method_line_id.payment_account_id
        )
        wizard = (
            self.env["bank.rec.widget"]
            .with_context(default_st_line_id=st_line.id)
        ...
        report = self.env.ref(
            "account_reports.bank_reconciliation_report"
    ...
    def test_reconciliation_report_transaction_without_statement(self):
        ...
        wizard = (
            self.env["bank.rec.widget"]
            .with_context(default_st_line_id=bank_statement_lines[1].id)
            .new({})
        )
        wizard._action_add_new_amls(payment_line, allow_partial=False)
        wizard._action_validate()
