...
class TestTaxReport(TestAccountReportsCommon):
    ...
    def setUpClass(cls):
        ...
        cls.init_invoice(
            ...
            post=True,
            amounts=[400],
            taxes=cls.test_fpos_tax_purchase,
        )
        cls.init_invoice(
            "in_refund",
            partner=cls.partner_a,
    ...
    def _add_basic_tax_for_report(
        ...
        for move_type_suffix in ("invoice", "refund"):
            ...
            base_report_line = cls._create_tax_report_line(
                ...
                tax_report,
                tag_name=base_report_line_name,
    ...
    def _assert_vat_closing(self, report, options, closing_vals_by_fpos):
        ...
        with patch.object(
            ...
            vat_closing_moves = self.env[
                "account.generic.tax.report.handler"
            ]._generate_tax_closing_entries(report, options)

            if len(options["companies"]) > 1:
                closing_moves_by_fpos = {
                    (move.company_id, move.fiscal_position_id): move
                    for move in vat_closing_moves
                }
            else:
                closing_moves_by_fpos = {
                    move.fiscal_position_id: move for move in vat_closing_moves
                }
            ...
            options,
            {
                self.foreign_vat_fpos: [
                    ...
                    # For sales operations
                    {
                        "debit": 90,
                        "credit": 0,
                        "account_id": self.tax_group_1.tax_receivable_account_id.id,
                    },
                    # For purchase operations
                    {
                        "debit": 0,
                        "credit": 80,
                        "account_id": self.tax_group_2.tax_payable_account_id.id,
                    },
                ]
            },
        )
    ...
    def test_vat_closing_button_availability(self):
        def assertTaxClosingAvailable(
            is_enabled, active_companies, export_main_company=None
        ):
            options = tax_report.with_context(
                allowed_company_ids=active_companies.ids
            ).get_options({})
            closing_button_dict = next(
                filter(
                    lambda x: x["action"] == "action_periodic_vat_entries",
                    options["buttons"],
                )
            )
            self.assertEqual(
                closing_button_dict.get("error_action"),
                None if is_enabled else "show_error_branch_allowed",
            )
            if is_enabled:
                self.assertEqual(
                    tax_report._get_sender_company_for_export(options),
                    export_main_company,
                )
        ...
        branch_1 = self.env["res.company"].create(
            {"name": "Branch 1", "parent_id": main_company.id}
        )
        ...
        branch_2 = self.env["res.company"].create(
            {"name": "Branch 2", "parent_id": main_company.id, "vat": "456"}
        )
        ...
        assertTaxClosingAvailable(
            True,
            main_company + branch_1 + branch_1_1 + branch_2 + branch_2_1,
            export_main_company=main_company,
        )
        assertTaxClosingAvailable(
            True,
            branch_2 + branch_2_1 + main_company + branch_1 + branch_1_1,
            export_main_company=branch_2,
        )
        assertTaxClosingAvailable(
            True, main_company + branch_1 + branch_1_1, export_main_company=main_company
        )
        assertTaxClosingAvailable(False, main_company + branch_1)
        assertTaxClosingAvailable(False, branch_1 + branch_1_1)
        assertTaxClosingAvailable(
            True, branch_1 + main_company + branch_1_1, export_main_company=main_company
        )
        assertTaxClosingAvailable(
            True, branch_2 + main_company + branch_2_1, export_main_company=branch_2
        )
    ...
    def test_caba_half_payment(self):
        ...
        def register_half_payment_for_invoice(invoice):
            ...
            payment_method_id = (
                ...
                else self.outbound_payment_method_line
            )
            self.env["account.payment.register"].with_context(
                active_ids=invoice.ids, active_model="account.move"
            ).create(
    ...
    def test_fiscal_position_switch_all_option_flow(self):
        ...
        to_check = self.basic_tax_report.get_options(
            {"fiscal_position": "all", "selected_variant_id": self.basic_tax_report.id}
        )
        self.assertEqual(
        ...
        self.foreign_vat_fpos.foreign_vat = (
            None  # No unlink because setupClass created some moves with it
        )
        to_check = self.basic_tax_report.get_options(
            {"fiscal_position": "all", "selected_variant_id": self.basic_tax_report.id}
        )
        ...
        to_check = foreign_tax_report.get_options(
            {"fiscal_position": "all", "selected_variant_id": foreign_tax_report.id}
        )
        self.assertEqual(
            to_check["fiscal_position"],
            foreign_vat_fpos.id,
        ...
        to_check = generic_tax_report.get_options(
            {
                "fiscal_position": foreign_vat_fpos.id,
        ...
        self.assertEqual(
            to_check["fiscal_position"],
            "all",
            "The generic report should always use 'all' fiscal position option.",
        )
    ...
    def test_tax_unit(self):
        ...
        for index, company in enumerate(all_companies):
            ...
            tax_group = self.env["account.tax.group"].search(
                [("company_id", "=", company.id), ("name", "=", "Test tax group")],
                limit=1,
            )
                ...
                (company_1, self.env["account.fiscal.position"]): [
                    {
                        "debit": 42,
                        "credit": 0,
                        "account_id": tax_accounts[company_1].id,
                    },
                    {
                        "debit": 0,
                        "credit": 42,
                ...
                (company_2, self.env["account.fiscal.position"]): [
                    {
                        "debit": 84,
                        "credit": 0,
                        "account_id": tax_accounts[company_2].id,
                    },
                    {
                        "debit": 0,
                        "credit": 84,
                        "account_id": tax_group_2.tax_payable_account_id.id,
                    },
                ],
            },
        )
    ...
    def test_tax_report_get_past_closing_entry(self):
        options = self._generate_options(
            self.basic_tax_report, "2021-01-01", "2021-12-31"
        )
    ...
    def setup_multi_vat_context(self):
        ...
        for tax in (
            ...
            base_line.tag_ids = get_positive_tag(
                local_tax_report_base_line
            ) + get_positive_tag(foreign_tax_report_base_line)
            tax_line.tag_ids = get_positive_tag(
                local_tax_report_tax_line
            ) + get_positive_tag(foreign_tax_report_tax_line)
    ...
    def test_engine_external_many_fiscal_positions(self):
        ...
        for fp in fiscal_positions:
            fp_options = self._generate_options(
                self.basic_tax_report_2.with_context(
        ...
        report_lines = self.basic_tax_report_2.with_company(company_2)._get_lines(
            report_options["all"]
        )
        self.assertLinesValues(
            # pylint: disable=bad-whitespace
    ...
    def test_tax_report_closing_entry_reset_to_draft(self):
        ...
        """
        options = self._generate_options(
            self.basic_tax_report, "2021-03-01", "2021-03-31"
        )
        vat_closing_action = (
            self.env["account.generic.tax.report.handler"]
            .with_context({"override_tax_closing_warning": True})
    ...
    def test_tax_report_closing_entry_draft_with_new_entries(self):
        ...
        subsequent_closing_entry = self.env["account.move"].browse(
            subsequent_vat_closing_action["res_id"]
        )
        self.assertRecordValues(
            subsequent_closing_entry.line_ids,
            [
                {
                    "account_id": initial_values[0]["account_id"],
                    "balance": initial_values[0]["balance"],
                },
                {
                    "account_id": initial_values[1]["account_id"],
                    "balance": initial_values[1]["balance"],
                },
            ],
        )
    ...
    def test_tax_report_multi_company_post_closing(self):
        ...
        branch_1 = self.env["res.company"].create(
            {"name": "Branch 1", "parent_id": root_company.id}
        )
        branch_1_1 = self.env["res.company"].create(
            {"name": "Branch 1.1", "parent_id": branch_1.id}
        )
        branch_2 = self.env["res.company"].create(
        ...
        for test_type, main_company, active_companies in [
            ("branches", root_company, branch_companies),
            ("tax units", tax_unit.main_company_id, tax_unit.company_ids),
        ]:
            with self.subTest(f"Post multicompany closing - {test_type}"):
                tax_report_with_companies = tax_report.with_context(
                    allowed_company_ids=active_companies.ids
                )
                options = self._generate_options(
                    tax_report_with_companies, "2023-01-01", "2023-01-01"
                )
                closing_moves = (
                    self.env["account.generic.tax.report.handler"]
                    .with_context(allowed_company_ids=active_companies.ids)
                    ._generate_tax_closing_entries(tax_report_with_companies, options)
                )

                self.assertEqual(
                    len(closing_moves),
                    len(active_companies),
                    "One closing move should have been created per company",
                )
                self.assertTrue(
                    all(move.state == "draft" for move in closing_moves),
                ...
                with self.enter_test_mode():
                    ...
                    self.assertTrue(
                        all(
                            move.state == "posted"
                            for move in (closing_moves - main_closing_move)
                        )
                    )
    ...
    def test_tax_report_prevent_draft_if_subsequent_posted(self):
        ...
        options = self._generate_options(
            self.basic_tax_report, "2023-04-01", "2023-06-30"
        )
        vat_closing_action = (
            self.env["account.generic.tax.report.handler"]
            .with_context({"override_tax_closing_warning": True})
        ...
        Q2_closing_entry = self.env[
            ...
            self.basic_tax_report, options, closing_moves=Q2_closing_entry
        )
        with self.enter_test_mode():
    ...
    def assert_period(self, input_date, expected_start, expected_end):
        period_start, period_end = self.env.company._get_tax_closing_period_boundaries(
            input_date, self.basic_tax_report
        )
        self.assertEqual(
            period_start,
            expected_start,
