...
class TestAccountReportsTaxReminder(TestAccountReportsCommon):
    ...
    def test_posting_adds_an_activity(self):
        ...
        self.assertFalse(
            all_report_activity_type
            & self.tax_return_move.activity_ids.activity_type_id,
        ...
        with patch.object(
            self.env.registry[self.report._name],
            "export_to_pdf",
            autospec=True,
        ...
        self.assertRecordValues(
            self.tax_return_move.activity_ids,
            [
                {
        ...
        self.assertRecordValues(
            next_tax_return_move.activity_ids,
            [
                {
                    "activity_type_id": act_type_report_to_send.id,
        ...
        self.assertFalse(
            all_report_activity_type
            & next_tax_return_move.activity_ids.activity_type_id,
    ...
    def test_tax_closing_activity_reminder_duplication(self):
        ...
        for i in range(0, 2):
            ...
            move.button_cancel()
        activity = self.env.company._get_tax_closing_reminder_activity(
    ...
    def test_tax_closing_activity_reminder_post(self):
        ...
        self.assertTrue(
            activity,
            "There has been no activity created for the current period closing",
        )
        with patch.object(
            self.env.registry[self.report._name],
            "export_to_pdf",
        ...
        _dummy, period_end = self.env.company._get_tax_closing_period_boundaries(
            fields.Date.from_string(self.options["date"]["date_to"])
            + relativedelta(days=1),
            self.report,
        )
        activity = self.env.company._get_tax_closing_reminder_activity(
            self.report.id, period_end
        )
        self.assertTrue(
            activity, "There has been no activity created for the next period closing"
        )
    ...
    def test_tax_closing_activity_reminder_reset_on_periodicity_change(self):
        ...
        new_activity = self.env.company._get_tax_closing_reminder_activity(
            self.report.id, period_end
        )
        ...
        self.assertEqual(
            fields.Date.from_string(
                new_activity.account_tax_closing_params["tax_closing_end_date"]
            ),
            period_end,
        )
