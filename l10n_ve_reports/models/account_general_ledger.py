...
class GeneralLedgerCustomHandler(models.AbstractModel):
    ...
    def _dynamic_lines_generator(
        ...
        totals_by_column_group = defaultdict(
            lambda: {"debit": 0, "credit": 0, "balance": 0}
        )
        for account, column_group_results in self._query_values(report, options):
            ...
            for column_group_key, results in column_group_results.items():
                ...
                account_debit = account_sum.get("debit", 0.0) + account_un_earn.get(
                    "debit", 0.0
                )
                account_credit = account_sum.get("credit", 0.0) + account_un_earn.get(
                    "credit", 0.0
                )
                account_balance = account_sum.get("balance", 0.0) + account_un_earn.get(
                    "balance", 0.0
                )

                eval_dict[column_group_key] = {
                    "amount_currency": account_sum.get("amount_currency", 0.0)
        ...
        if (
            ...
            and len(journal_options) == 1
            and journal_options[0]["type"] in ("sale", "purchase")
        ):
            lines += self._tax_declaration_lines(
                report, options, journal_options[0]["type"]
            )
    ...
    def _custom_unfold_all_batch_data_generator(
        ...
        limit_to_load = (
            ...
            if report.load_more_limit and not options.get("export_mode")
            else None
        )
        has_more_per_account_id = {}

        unlimited_aml_results_per_account_id = self._get_aml_values(
            report, options, account_ids_to_expand
        )[0]
        if limit_to_load:
            ...
            for (
                account_id,
                account_aml_results,
            ) in unlimited_aml_results_per_account_id.items():
                account_values = {}
        ...
        return {
            "initial_balances": self._get_initial_balance_values(
                report, account_ids_to_expand, options
            ),
    ...
    def _tax_declaration_lines(self, report, options, tax_type):
        ...
        rslt = [
            {
                "id": report._get_generic_line_id(
                    None, None, markup="tax_decl_header_1"
                ),
            ...
            },
            {
                ...
                "name": _("Name"),
                "columns": [
                    {"name": labels_replacement.get(col["expression_label"], "")}
        ...
        tax_report_options = generic_tax_report.get_options(
            {
                **options,
        ...
        tax_type_parent_line_id = generic_tax_report._get_generic_line_id(
            None, None, markup=tax_type
        )
        ...
        for tax_report_line in tax_report_lines:
            if tax_report_line.get("parent_id") == tax_type_parent_line_id:
                ...
                tax_report_line["columns"] = [
                    row_column_map.get(col["expression_label"], {})
                    for col in options["columns"]
                ]
    ...
    def _query_values(self, report, options):
                ...
                if equity_unaffected_account := unaffected_earnings_accounts.filtered(
                    ...
                    for column_group_key in options["column_groups"]:
                        groupby_accounts.setdefault(
                            ...
                            {
                                col_group_key: {"unaffected_earnings": {}}
                                for col_group_key in options["column_groups"]
                            },
                        )
                        if unaffected_earnings := groupby_company.get(column_group_key):
                            if groupby_accounts[equity_unaffected_account.id][
                                column_group_key
                            ].get("unaffected_earnings"):
                                for key in [
                                    "amount_currency",
                                    "debit",
                                    "credit",
                                    ...
                                    groupby_accounts[equity_unaffected_account.id][
                                        column_group_key
                                    ]["unaffected_earnings"][
                                        key
                                    ] += unaffected_earnings[key]
                            else:
                                groupby_accounts[equity_unaffected_account.id][
                                    column_group_key
                                ]["unaffected_earnings"] = unaffected_earnings
        ...
        if groupby_accounts:
            accounts = self.env["account.account"].search(
                [("id", "in", list(groupby_accounts.keys()))]
            )
        else:
            accounts = []
    ...
    def _get_query_sums(self, report, options) -> SQL:
        ...
        for column_group_key, options_group in options_by_column_group.items():
            ...
            if options_group.get("export_mode") == "print" and options_group.get(
                "filter_search_bar"
            ):
                query_domain.append(
    ...
    def _get_options_unaffected_earnings(self, options):
        ...
        new_date_to = (
            fields.Date.from_string(new_options["date"]["date_to"])
            if options.get("include_current_year_in_unaff_earnings")
            else fiscalyear_dates["date_from"] - timedelta(days=1)
        )

        new_options["date"] = self.env["account.report"]._get_dates_period(
            None, new_date_to, "single"
        )
    ...
    def _get_aml_values(
        ...
        for aml_result in self._cr.dictfetchall():
            ...
            if aml_key not in account_result:
                account_result[aml_key] = {
                    col_group_key: {} for col_group_key in options["column_groups"]
                }

            already_present_result = account_result[aml_key][
            ...
            if already_present_result:
                ...
                already_present_result["amount_currency"] += aml_result[
                    "amount_currency"
                ]
            else:
    ...
    def _get_initial_balance_values(self, report, account_ids, options):
        ...
        for column_group_key, options_group in report._split_options_per_column_group(
            ...
            query = report._get_report_query(
                new_options, "from_beginning", domain=domain
            )
            queries.append(
                SQL(
                    """
    ...
    def _get_options_initial_balance(self, options):
        ...
        new_options = options.copy()
        date_to = (
            new_options["comparison"]["periods"][-1]["date_from"]
            if new_options.get("comparison", {}).get("periods")
            else new_options["date"]["date_from"]
        )
        ...
        if date_from == current_fiscalyear_dates["date_from"]:
            ...
            previous_fiscalyear_dates = self.env.company.compute_fiscalyear_dates(
        ...
        new_options["include_current_year_in_unaff_earnings"] = (
            include_current_year_in_unaff_earnings
        )
    ...
    def _get_account_title_line(self, report, options, account, has_lines, eval_dict):
        line_columns = []
        for column in options["columns"]:
            col_value = eval_dict.get(column["column_group_key"], {}).get(
            ...
            value = (
                ...
                or (col_expr_label == "amount_currency" and not account.currency_id)
                else col_value
            )
        ...
        return {
            ...
            "unfolded": has_lines
            and (is_in_unfolded_lines or options.get("unfold_all")),
            "expand_function": "_report_expand_unfoldable_line_general_ledger",
        }

    def _get_aml_line(
        self, report, parent_line_id, options, eval_dict, init_bal_by_col_group
    ):
        line_columns = []
        for column in options["columns"]:
            col_expr_label = column["expression_label"]
            ...
            if col_value is not None:
                if col_expr_label == "amount_currency":
                    ...
                    col_value = (
                        None
                        if col_currency == self.env.company.currency_id
                        else col_value
                    )
        ...
        return {
            "id": report._get_generic_line_id(
                "account.move.line", aml_id, parent_line_id=parent_line_id, markup=date
            ),
    ...
    def _report_expand_unfoldable_line_general_ledger(
        ...
        if model != "account.account":
            raise UserError(
                _("Wrong ID for general ledger line to expand: %s", line_dict_id)
            )
        ...
        if offset == 0:
            if unfold_all_batch_data:
                account, init_balance_by_col_group = unfold_all_batch_data[
                    "initial_balances"
                ][model_id]
            else:
                account, init_balance_by_col_group = self._get_initial_balance_values(
                    report, [model_id], options
                )[model_id]
        ...
        for aml_result in aml_results.values():
            new_line = self._get_aml_line(
                report, line_dict_id, options, aml_result, next_progress
            )
