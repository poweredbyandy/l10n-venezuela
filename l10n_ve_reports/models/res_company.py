...
class ResCompany(models.Model):
    ...
    def _get_and_update_tax_closing_moves(
        ...
        for fpos in itertools.chain(
            fiscal_positions, [False] if include_domestic else []
        ):
            ...
            ref = _(
                ...
                period=self._get_tax_closing_move_description(
            ...
            activity = self._get_tax_closing_reminder_activity(
                report.id, period_end, fpos_id
            )
            tax_closing_options = tax_closing_move._get_tax_closing_report_options(
                tax_closing_move.company_id,
                tax_closing_move.fiscal_position_id,
                tax_closing_move.tax_closing_report_id,
                tax_closing_move.date,
            )
            if (
                not activity
                and report._get_sender_company_for_export(tax_closing_options)
                == tax_closing_move.company_id
            ):
    ...
    def _get_tax_closing_report_display_name(self, report):
        if report.get_external_id().get(report.id) in (
    ...
    def _generate_tax_closing_reminder_activity(
        ...
        tax_closing_activity_type = self.env.ref(
            "l10n_ve_reports.tax_closing_activity_type"
        )
        ...
        summary = _(
            ...
            report_label=self._get_tax_closing_report_display_name(report),
            period=self._get_tax_closing_move_description(
                periodicity, period_start, period_end, fiscal_position, report
            ),
        )

        activity_user = (
            tax_closing_activity_type.default_user_id
            if tax_closing_activity_type
            else self.env["res.users"]
        )
        if activity_user and not (
            self in activity_user.company_ids
            and activity_user.has_group("account.group_account_manager")
        ):
    ...
    def _get_tax_closing_reminder_activity(self, report_id, period_end, fpos_id=False):
        ...
        return self._get_tax_closing_journal().activity_ids.filtered(
            lambda act: act.account_tax_closing_params
            and (
                act.activity_type_id == tax_closing_activity_type
    ...
    def _get_tax_closing_move_description(
        ...
        if foreign_vat_fpos_count:
            if fiscal_position:
                ...
                state_codes = (
                    [self.state_id.code]
                    if self.state_id and vat_fpos_with_state_count
                    else []
                )
