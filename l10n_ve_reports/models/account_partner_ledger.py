...
class PartnerLedgerCustomHandler(models.AbstractModel):
    ...
    def _get_custom_display_config(self):
        return {
            ...
            "components": {
                "AccountReportLineCell": "l10n_ve_reports.PartnerLedgerLineCell",
            },
            "templates": {
                "AccountReportFilters": "l10n_ve_reports.PartnerLedgerFilters",
                "AccountReportLineName": "l10n_ve_reports.PartnerLedgerLineName",
            },
        }
    ...
    def _dynamic_lines_generator(
        ...
        partner_lines, totals_by_column_group = self._build_partner_lines(
            report, options
        )
    ...
    def _build_partner_lines(self, report, options, level_shift=0):
        ...
        totals_by_column_group = {
            column_group_key: {
                total: 0.0 for total in ["debit", "credit", "amount", "balance"]
            }
    ...
    def _report_expand_unfoldable_line_partner_ledger_prefix_group(
        ...
        partner_lines, dummy = self._build_partner_lines(
            report, expand_options, level_shift=parent_level
        )

        for partner_line in partner_lines:
            partner_line["id"] = report._build_subline_id(
    ...
    def _custom_options_initializer(self, report, options, previous_options):
        ...
        if not self.env.ref(
            ...
            columns_to_hide = []
            options["hide_account"] = (previous_options or {}).get(
                "hide_account", False
            )
            columns_to_hide += (
                ["journal_code", "account_code", "matching_number"]
                if options["hide_account"]
                else []
            )

            options["hide_debit_credit"] = (previous_options or {}).get(
                "hide_debit_credit", False
            )
            columns_to_hide += (
                ["debit", "credit"] if options["hide_debit_credit"] else ["amount"]
            )
    ...
    def action_send_statements(self, options):
        ...
        template = self.env.ref(
            "l10n_ve_reports.email_template_customer_statement", False
        )
    ...
    def action_open_partner(self, options, params):
        dummy, record_id = self.env["account.report"]._get_model_info_from_id(
            params["id"]
        )
        return {
            "type": "ir.actions.act_window",
    ...
    def _query_partners(self, report, options):
        ...
        def assign_sum(row):
            ...
            if any(
                ...
                for field in fields_to_assign:
                    groupby_partners[row["groupby"]][row["column_group_key"]][
                        field
                    ] += row[field]
        ...
        if None in groupby_partners:
            ...
            for column_group_key in options["column_groups"]:
                ...
                groupby_partners[None][column_group_key]["credit"] += totals["debit"][
                    column_group_key
                ]
                groupby_partners[None][column_group_key]["amount"] += totals["amount"][
                    column_group_key
                ]
                groupby_partners[None][column_group_key]["balance"] -= totals[
                    "balance"
                ][column_group_key]
    ...
    def _get_options_initial_balance(self, options):
        ...
        new_date_options = dict(
            options["date"], date_from=False, date_to=fields.Date.to_string(new_date_to)
        )
    ...
    def _get_aml_values(self, options, partner_ids, offset=0, limit=None):
        ...
        if partner_ids_wo_none:
            ...
            indirectly_linked_aml_partner_clause = SQL(
                "aml_with_partner.partner_id IN %s", tuple(partner_ids_wo_none)
            )
    ...
    def _get_report_line_partners(
        ...
        return {
            "id": line_id,
            "name": partner is not None
            and (partner.name or "")[:128]
