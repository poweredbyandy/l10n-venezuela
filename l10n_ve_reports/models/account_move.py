...
class AccountMove(models.Model):
    ...
    def action_post(self):
        ...
        try:
            ...
            return {
                ...
                "tag": "l10n_ve_reports.redirect_action",
                "target": "new",
                "name": "Depending Action",
                "params": {
                    "depending_action": exception.args[0],
                    "message": _(
                        "It seems there is some depending closing move to be posted"
                    ),
    ...
    def button_draft(self):
        ...
        for closing_move in self.filtered(lambda m: m.tax_closing_report_id):
            ...
            options = closing_move._get_tax_closing_report_options(
                closing_move.company_id,
                closing_move.fiscal_position_id,
                report,
                closing_move.date,
            )
            closing_months_delay = (
                closing_move.company_id._get_tax_periodicity_months_delay(report)
            )
            ...
            carryover_impacted_period_end = fields.Date.from_string(
                options["date"]["date_to"]
            ) + relativedelta(months=closing_months_delay)
            tax_lock_date = closing_move.company_id.tax_lock_date
            if (
                carryover_values
                and tax_lock_date
                and tax_lock_date >= carryover_impacted_period_end
            ):
                raise UserError(
                    _(
    ...
    def _get_tax_to_pay_on_closing(self):
        ...
        payable_lines = self.line_ids.filtered(
            lambda line: line.account_id in tax_payable_accounts
        )
    ...
    def _close_tax_period(self, report, options):
        ...
        options = self._get_tax_closing_report_options(
            self.company_id, self.fiscal_position_id, report, self.date
        )

        if not self.fiscal_position_id and (
            ...
            self.env["account.report"]._generate_default_external_values(
                options["date"]["date_from"], options["date"]["date_to"], True
            )
        ...
        if sender_company == self.company_id:
            ...
            depending_closings_to_post = depending_closings.filtered(
                lambda x: x.state == "draft"
            )
            if depending_closings_to_post:
                depending_action = self.env["ir.actions.actions"]._for_xml_id(
                    "account.action_move_journal_line"
                )
                depending_action = clean_action(depending_action, env=self.env)

                if len(depending_closings_to_post) == 1:
                    depending_action["views"] = [
                        (self.env.ref("account.view_move_form").id, "form")
                    ]
                    depending_action["res_id"] = depending_closings_to_post.id
                else:
                    depending_action["domain"] = [
                        ("id", "in", depending_closings_to_post.ids)
                    ]
                    depending_action["context"] = dict(
                        ast.literal_eval(depending_action["context"])
                    )
            ...
            self.with_context(no_new_invoice=True).message_post(
                body=self.ref, subject=subject, attachments=attachments
            )
            ...
            # End activity
            activity = self.company_id._get_tax_closing_reminder_activity(
                report.id, self.date, self.fiscal_position_id.id
            )
            if activity:
    ...
    def _close_tax_period_create_activities(self):
        ...
        mat_to_pay_xml_id = "l10n_ve_reports.mail_activity_type_tax_report_to_pay"
        mat_to_pay = self.env.ref(mat_to_pay_xml_id, raise_if_not_found=False)

        act_user = mat_to_send.default_user_id
        if act_user and not (
            self.company_id in act_user.company_ids
            and act_user.has_group("account.group_account_manager")
        ):
        ...
        moves_without_send_activity = self.filtered_domain(
            [
                ...
                (
                    "activity_ids",
                    "not any",
                    [("activity_type_id.id", "=", mat_to_send.id)],
                ),
            ]
        )
        ...
        for move in moves_without_send_activity:
            period_start, period_end = (
                move.company_id._get_tax_closing_period_boundaries(
                    move.date, move.tax_closing_report_id
                )
            )
            period_desc = move.company_id._get_tax_closing_move_description(
                move.company_id._get_tax_periodicity(move.tax_closing_report_id),
                period_start,
                period_end,
                move.fiscal_position_id,
                move.tax_closing_report_id,
            )
            ...
            if (
                mat_to_pay
                and mat_to_pay not in move.activity_ids.activity_type_id
    ...
    def refresh_tax_entry(self):
        for move in self.filtered(
            lambda m: m.tax_closing_report_id and m.state == "draft"
        ):
            report = move.tax_closing_report_id
            options = move._get_tax_closing_report_options(
                move.company_id, move.fiscal_position_id, report, move.date
    ...
    @api.model
    def _get_tax_closing_report_options(
        self, company, fiscal_position, report, date_inside_period
    ):
        _dummy, date_to = company._get_tax_closing_period_boundaries(
            date_inside_period, report
        )
        ...
        if report.filter_multi_company == "tax_units":
            ...
            candidate_tax_unit = company.account_tax_unit_ids.filtered(
                lambda x: x.country_id == report_country
            )
        ...
        return report.with_context(allowed_company_ids=company_ids).get_options(
