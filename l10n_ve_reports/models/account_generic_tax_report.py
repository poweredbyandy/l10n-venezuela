...
class AccountTaxReportHandler(models.AbstractModel):
    ...
    def _custom_options_initializer(self, report, options, previous_options):
        ...
        options["show_tax_period_filter"] = (
            periodicity not in optional_periods or day != 1 or month != 1
        )
        if (
            not options["show_tax_period_filter"]
            and "custom" not in options["date"]["filter"]
        ):
            period_type = optional_periods[periodicity]
            options["date"]["filter"] = options["date"]["filter"].replace(
                "tax_period", period_type
            )
            options["date"]["period_type"] = options["date"]["period_type"].replace(
                "tax_period", period_type
            )

    def _get_custom_display_config(self):
        display_config = defaultdict(dict)
        display_config["templates"]["AccountReportFilters"] = (
            "l10n_ve_reports.GenericTaxReportFiltersCustomizable"
        )
    ...
    def _customize_warnings(
        ...
        if "l10n_ve_reports.common_warning_draft_in_period" in warnings:
            ...
            if not self.env["account.move"].search_count(
                ...
                warnings.pop("l10n_ve_reports.common_warning_draft_in_period")
        ...
        if rows:
            warnings["l10n_ve_reports.tax_report_warning_inactive_tags"] = {}
    ...
    def _get_periodic_vat_entries(self, options, from_post=False):
        ...
        companies_moves = self._get_tax_closing_entries_for_closed_period(
            report, options, companies, posted_only=False
        )
        moves += companies_moves
        moves += self._generate_tax_closing_entries(
            report,
            options,
            companies=companies - companies_moves.company_id,
            from_post=from_post,
        )

        return moves

    def _generate_tax_closing_entries(
        self, report, options, closing_moves=None, companies=None, from_post=False
    ):
        ...
        companies_without_closing = companies.filtered(
            lambda company: company not in closing_moves.company_id
        )
        if closing_moves:
        ...
        for company in companies_without_closing:
            include_domestic, fiscal_positions = self._get_fpos_info_for_tax_closing(
                company, report, options
            )
            company_closing_moves = company._get_and_update_tax_closing_moves(
                end_date,
                report,
    ...
    def _get_tax_closing_entries_for_closed_period(
        ...
        for company in companies:
            _dummy, period_end = company._get_tax_closing_period_boundaries(
                fields.Date.from_string(options["date"]["date_to"]), report
            )
            include_domestic, fiscal_positions = self._get_fpos_info_for_tax_closing(
                company, report, options
            )
            fiscal_position_ids = fiscal_positions.ids + (
                [False] if include_domestic else []
            )
            state_domain = (
                ("state", "=", "posted") if posted_only else ("state", "!=", "cancel")
            )
    ...
    def _compute_vat_closing_entry(self, company, options):
        ...
        self = self.with_company(
            company
        )  # Needed to handle access to property fields correctly
    ...
    def _add_tax_group_closing_items(self, tax_group_subtotal, closing_move):
        ...
        for key, value in tax_group_subtotal.items():
            ...
            if key[2] and key[2] not in account_already_balanced:
                total += _add_line(
                    key[2], _("Balance tax current account (payable)"), currency
                )
    ...
    def action_open_amls_with_archived_tags(self, options, params=None):
        return {
            ...
            "views": [
                (self.env.ref("l10n_ve_reports.view_archived_tag_move_tree").id, "list")
            ],
        }
...
class GenericTaxReportCustomHandler(models.AbstractModel):
    ...
    def _get_custom_display_config(self):
        ...
        parent_config["templates"]["AccountReportLineName"] = (
            "l10n_ve_reports.TaxReportLineName"
        )
    ...
    def _custom_options_initializer(self, report, options, previous_options=None):
        ...
        if (
            ...
            <= (0 if options["allow_domestic"] else 1)
    ...
    def _build_report_line(
        ...
        if groupby_key == "src_tax_type_tax_use":
            ...
            if options.get("multi-company"):
                report_line["name"] = (
                    f"{report_line['name']} - {tax.company_id.display_name}"
                )
    ...
    def _check_line_consistency(self, report, options, report_line, tax, warnings=None):
        ...
        for (
            ...
            if is_inconsistent:
                error = abs(abs(tax_value) - abs(computed_tax_amount)) / float(
                    net_value or 1
                )
                ...
                if error > 0.001:
                    ...
                    warnings["l10n_ve_reports.tax_report_warning_lines_consistency"] = {
                        "alert_type": "danger"
                    }
