...
class TrialBalanceCustomHandler(models.AbstractModel):
    ...
    def _dynamic_lines_generator(
        ...
        def _update_column(line, column_key, new_value):
            ...
            line["columns"][column_key]["is_zero"] = (
                self.env.company.currency_id.is_zero(new_value)
            )

        def _update_balance_columns(
            line, debit_column_key, credit_column_key, balance_column_key=None
        ):
            debit_value = (
                line["columns"][debit_column_key]["no_format"]
                if debit_column_key is not None
                else False
            )
            credit_value = (
                line["columns"][credit_column_key]["no_format"]
                if credit_column_key is not None
                else False
            )
        ...
        init_balance_credit_index = next(
            (
                index
                for index, column in enumerate(options["columns"])
                if column.get("expression_label") == "credit"
            ),
            None,
        )

        end_balance_debit_index = next(
            (
                index
                for index, column in enumerate(options["columns"])
                if column.get("expression_label") == "debit"
                and column.get("column_group_key") == TRIAL_BALANCE_END_COLUMN_GROUP_KEY
            ),
            None,
        )
        end_balance_credit_index = next(
            (
                index
                for index, column in enumerate(options["columns"])
                if column.get("expression_label") == "credit"
        ...
        end_balance_balance_index = next(
            (
                index
                for index, column in enumerate(options["columns"])
        ...
        for line in lines[:-1]:
            # Initial balance
            _update_balance_columns(
                line, init_balance_debit_index, init_balance_credit_index
            )
            ...
            if end_balance_debit_index is not None:
                end_balance_debit_sum = sum(
                    ...
                    if column.get("expression_label") == "debit"
                    and index != end_balance_debit_index
                    and column["no_format"] is not None
                )
            ...
            if end_balance_credit_index is not None:
                end_balance_credit_sum = sum(
                    ...
                    for index, column in enumerate(line["columns"])
                    if column.get("expression_label") == "credit"
                    and index != end_balance_credit_index
                    and column["no_format"] is not None
                )
            ...
            _update_balance_columns(
                line,
                end_balance_debit_index,
                end_balance_credit_index,
                end_balance_balance_index,
            )
        ...
        if lines:
            ...
            for index in (
                init_balance_debit_index,
                init_balance_credit_index,
                end_balance_debit_index,
                end_balance_credit_index,
            ):
                if index is not None:
                    total_line["columns"][index]["no_format"] = sum(
                        currency.round(line["columns"][index]["no_format"])
                        for line in lines[:-1]
                        if report._get_model_info_from_id(line["id"])[0]
                        == "account.account"
                    )
    ...
    def _create_and_append_column_group(
        ...
        side_to_append,
        group_vals,
        exclude_initial_balance=False,
        append_col_groups=True,
    ):
        header_element = [{"name": header_name, "forced_options": forced_options}]
        column_headers = [header_element, *options["column_headers"][1:]]
        column_group_vals = report._generate_columns_group_vals_recursively(
            column_headers, group_vals
        )
        ...
        columns, column_groups = report._build_columns_from_column_group_vals(
            forced_options, column_group_vals
        )
    ...
    def _custom_options_initializer(self, report, options, previous_options):
        ...
        for function, side in self._get_column_group_creation_data(
            report, options, previous_options
        ):
            function(
                report,
                options,
                previous_options,
                default_group_vals,
                left_side if side == "left" else right_side,
            )

        # Update options
        options["column_headers"][0] = (
            left_side["column_headers"]
            + options["column_headers"][0]
            + right_side["column_headers"]
        )
        options["column_groups"].update(left_side["column_groups"])
        options["column_groups"].update(right_side["column_groups"])
        options["columns"] = (
            left_side["columns"] + options["columns"] + right_side["columns"]
        )
        options["ignore_totals_below_sections"] = (
        ...
        if options.get("hierarchy"):
            for line in lines:
                ...
                if model == "account.group":
                    ...
                    line["class"] = (
                        line_classes + " o_account_coa_column_contrast_hierarchy"
                    )

        return lines

    def _create_column_group_initial_balance(
        ...
        initial_forced_options = {
            ...
            "include_current_year_in_unaff_earnings": initial_balance_options[
    ...
    def _create_column_group_end_balance(
        self, report, options, previous_options, default_group_vals, side_to_append
    ):
        ...
        compute_end_balance = bool(
            options.get("analytic_accounts_groupby")
            or options.get("analytic_plans_groupby")
            or options.get("selected_horizontal_group_id")
        )
        self._create_and_append_column_group(
            report,
            options,
