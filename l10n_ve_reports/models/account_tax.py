...
class AccountTaxUnit(models.Model):
    ...
    def create(self, vals_list):
        ...
        generic_ec_sales_report = self.env.ref(
            "l10n_ve_reports.generic_ec_sales_report"
        )
    ...
    def _compute_fiscal_position_completion(self):
        for unit in self:
            ...
            for company in unit.company_ids:
                origin_company = (
                    company._origin if isinstance(company.id, models.NewId) else company
                )
                ...
                if create_or_refresh:
                    ...
                    data = {
                        ...
                        "values": {
                            ...
                            + [
                                Command.create({"tax_src_id": tax.id})
                                for tax in taxes_to_map
                            ],
                        },
                    }
        ...
        for record in self:
            ...
            if not checked_country_code:
                ...
                error_message = self.env["res.partner"]._build_vat_error_message(
                    record.country_id.code.lower(), record.vat, tu_label
                )
