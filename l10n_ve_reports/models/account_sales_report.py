...
class ECSalesReportCustomHandler(models.AbstractModel):
    _name = "account.ec.sales.report.handler.oca"
    ...
    def _get_custom_display_config(self):
        return {
            "components": {
                "AccountReportFilters": "l10n_ve_reports.SalesReportFilters",
            },
        }
        ...
        def assign_sum(row):
            ...
            if not company_currency.is_zero(row["balance"]):
                ...
                if row["tax_element_id"] in options["sales_report_taxes"]["goods"]:
                    ...
                    row["tax_element_id"] in options["sales_report_taxes"]["triangular"]
                ):
                ...
                if warnings is not None:
                    if row["country_code"] not in self._get_ec_country_codes(options):
                        warnings[
                            "l10n_ve_reports.sales_report_warning_non_ec_country"
                        ] = {"alert_type": "warning"}
                    elif not row.get("vat_number"):
                        warnings["l10n_ve_reports.sales_report_warning_missing_vat"] = {
                            "alert_type": "warning"
                        }
                    if row.get("same_country") and row["country_code"]:
                        warnings[
                            "l10n_ve_reports.sales_report_warning_same_country"
                        ] = {"alert_type": "warning"}
                    if duplicated_vat:
                        if warnings.get(
                            "l10n_ve_reports.sales_report_warning_duplicated_vat"
                        ):
                            warnings[
                                "l10n_ve_reports.sales_report_warning_duplicated_vat"
    ...
    def _get_tag_ids_filtered(self, options):
        ...
        for operation_type in options.get("ec_tax_filter_selection", []):
            if operation_type.get("selected"):
                allowed_taxes.update(
                    options["sales_report_taxes"][operation_type.get("id")]
                )
    ...
    def _get_duplicated_vat_partners(self, duplicated_partners_vat):
        view_ref = self.env.ref(
            "l10n_ve_reports.duplicated_vat_partner_tree_view", raise_if_not_found=False
        )
        return {
