...
class TestChartTemplateVe(L10nVeSeniatCommon):
    ...
    def test_ve_seniat_res_company_writeoff_accounts(self):
        tpl = self.env["account.chart.template"]
        funcs = tpl._template_register["ve_seniat"]["res.company"]
        row = funcs[0](tpl)[self.env.company.id]
        self.assertEqual(
            row["account_journal_early_pay_discount_loss_account_id"],
            "account_account_5102015",
        )
        self.assertEqual(
            row["account_journal_early_pay_discount_gain_account_id"],
            "account_account_4102006",
        )
        company = self.env.company
        self.assertEqual(
            company.account_journal_early_pay_discount_loss_account_id.code,
            "5102015",
        )
        self.assertEqual(
            company.account_journal_early_pay_discount_gain_account_id.code,
            "4102006",
        )
